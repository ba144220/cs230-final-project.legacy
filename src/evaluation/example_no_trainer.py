...
def main():
    ...
    dummy_dataset = load_dummy_dataset(dataset_args)
    print("Preprocessing data...")
    batches = data_preprocessing(dummy_dataset, tokenizer, generation_args.batch_size)
    ...
    for i, result in enumerate(inference_results):
        ...
        print(f"Expected: {dummy_dataset[i]['answer']}")
        if result == dummy_dataset[i]['answer']:
            correct_count += 1
        print()
    print("-" * 100)
